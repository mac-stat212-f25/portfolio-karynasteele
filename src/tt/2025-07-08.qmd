...
ggplot(fide_ratings_september1, aes(x = fed, y = avg_rating)) +
  ...
  theme_classic()
```
```{r, Horizontal bar chart showing average chess ratings by federation for countries with more than 2500 players. Bars are colored with a gradient from light blue to dark blue based on rating. The federation with the highest average rating is annotated in red.}
